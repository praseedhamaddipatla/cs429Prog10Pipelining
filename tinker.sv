...
module tinker_core (
  ...
  // ---------------------------------------------------------------------------
  wire       stall = rob_full || rs_full || fp_full || lsq_full || branch_inflight;
  ...
  memory #(
      ...
      .instr_out0 (mem_i0),
      .instr_out1 (mem_i1),
      .data_addr  (call_pending ? call_addr : ret_pending ? ret_addr : ld_addr_reg),
  ...
  wire [63:0] alu_act_tgt_w = alu_ibrimm_p1 ? (alu_pc_p1 + alu_b_p1)  // brr L  — pc-relative imm
  ...
  always @(*) begin
    ...
    if (lsq_v[sf] && lsq_st[sf] && lsq_ardy[sf] && lsq_drdy[sf] &&
      (lsq_rob[sf] < lsq_rob[lsq_head]) &&   // ONLY older stores
  ...
  always @(posedge clk) begin
    if (reset) begin
      ...
      if (c0en && rob_valid[c0rob]) begin
        ...
        // --- PATCH 1D: clear branch_inflight on resolution ---
        if (alu_ibr_p1 || alu_ijmp_p1) branch_inflight <= 0;
      ...
      if (!redirect_en && lsq_cnt > 0 && lsq_v[lsq_head] && lsq_ardy[lsq_head]) begin
        if (lsq_ld[lsq_head]) begin
          ld_addr_reg     <= lsq_base[lsq_head] + lsq_imm[lsq_head];
          ld_fire_d1      <= 1;
          ld_done         <= ld_fire_d1;
          ld_fire_d1      <= 0;
      ...
      if (!redirect_en && !call_pending && !ret_pending && !flush_this_cycle) begin : dispatch_blk
        ...
        // -------- instr 0 --------
        if (d0_en) begin
          if (d0_br || d0_brgt || d0_jmp) branch_inflight <= 1;
          p0_ps    = rat_map[d0_rs];
          ...
          if (d0_mem) begin
            ...
            if (prf_rdy[p0_ps]) begin
              rs_psrdy[rs_free_slot] <= 1;
              rs_vs[rs_free_slot]    <= prf[p0_ps];
            end
            if (prf_rdy[p0_pt]) begin
              rs_ptrdy[rs_free_slot] <= 1;
              rs_vt[rs_free_slot]    <= prf[p0_pt];
            end
        ...
        if (d1_en) begin
          ...
          if (d1_mem) begin
            ...
            begin : rs_slot1
              ...
              rs_v[rslot] <= 1;
              if (prf_rdy[p1_ps]) begin
                rs_psrdy[rslot] <= 1;
                rs_vs[rslot]    <= prf[p1_ps];
              end
              if (prf_rdy[p1_pt]) begin
                rs_ptrdy[rslot] <= 1;
                rs_vt[rslot]    <= prf[p1_pt];
              end
              rs_op[rslot]     <= d1_op;
              rs_ps[rslot]     <= p1_ps;
              rs_pt[rslot]     <= p1_pt;
