...
// Based on working single-issue baseline with dual ALU/FPU, 64-byte fetch, FPU stash added
...
module tinker_core (
  ...
  // Second LSQ execute slot: dual-execute two consecutive loads only
  wire [2:0] lsq_head2 = lsq_head + 3'd1;
  wire lsq_exec2 = !redirect_en && lsq_exec && lsq_cnt > 1 &&
                   lsq_v[lsq_head2] && lsq_ardy[lsq_head2] &&
                   lsq_ld[lsq_head2] && lsq_ld[lsq_head];

  // 4-entry decode queue (64-byte fetch)
  reg dq_v[0:3];
  reg [31:0] dq_i[0:3];
  reg [63:0] dq_pc[0:3];
  reg dq_ptaken[0:3];
  ...
  reg alu_imovr_p, alu_imovi_p, alu_ical_p, alu_iret_p;
  reg        alu_ptaken_p;
  reg [63:0] alu_ptgt_p;
  ...
  fpu u_fpu1 (
      ...
      .valid_out(fpu1_vout),
      .result(fpu1_res),
      .rob_tag_out(fpu1_tout)
  );

  reg [63:0] pc_reg;
  wire [31:0] mem_i0, mem_i1, mem_i2, mem_i3;
  reg [63:0] lsq_maddr, lsq_mwdata;
  reg         lsq_mwe;
  ...
  branch_predictor u_bp0 (
      ...
      .reset(reset),
      .pc(pc_reg),
      .pred_taken(bp_pred_taken0),
      .pred_target(bp_pred_target0),
      .btb_hit(bp_hit0),
      .upd_en(bp_upd_en),
      .upd_pc(bp_upd_pc),
  ...
  branch_predictor u_bp1 (
      ...
      .reset(reset),
      .pc(pc_reg + 64'd4),
      .pred_taken(bp_pred_taken1),
      .pred_target(bp_pred_target1),
      .btb_hit(bp_hit1),
      .upd_en(bp_upd_en),
      .upd_pc(bp_upd_pc),
      .upd_taken(bp_upd_taken),
  ...
  // ── CDB0: ALU0 (with 1-cycle pipeline registers) ──
  reg [PHYS_W-1:0] alu_pd_p1;
  reg [63:0] alu_vs_p1, alu_pc_p1, alu_b_p1, alu_ibgt_tgt_p1;
  reg alu_imovr_p1, alu_imovi_p1, alu_ical_p1, alu_iret_p1;
  reg alu_ibr_p1, alu_ijmp_p1, alu_ibgt_p1, alu_ibrreg_p1, alu_ibrimm_p1;

  // ── CDB2: ALU1 (with 1-cycle pipeline registers) ──
  ...
  wire c0en = alu_vout;
  wire [PHYS_W-1:0] c0pd = alu_pd_p1;
  wire [ROB_BITS-1:0] c0rob = alu_tout[ROB_BITS-1:0];
  wire [63:0] c0val = alu_imovr_p1  ? alu_vs_p1
  ...
  wire [63:0] alu_act_tgt_w =
      alu_ibrimm_p1 ? (alu_pc_p1 + alu_b_p1) : alu_ibrreg_p1 ? (alu_pc_p1 + alu_vs_p1)
    : alu_ibgt_p1   ? alu_ibgt_tgt_p1 : alu_ibr_p1 ? alu_b_p1 : alu_vs_p1;

  wire c2en = alu1_vout;
  wire [PHYS_W-1:0] c2pd = alu1_pd_p1;
  wire [ROB_BITS-1:0] c2rob = alu1_tout[ROB_BITS-1:0];
  wire [63:0] c2val = alu1_imovr_p1  ? alu1_vs_p1
  ...
  wire [63:0] alu1_act_tgt_w =
      alu1_ibrimm_p1 ? (alu1_pc_p1 + alu1_b_p1) : alu1_ibrreg_p1 ? (alu1_pc_p1 + alu1_vs_p1)
    : alu1_ibgt_p1   ? alu1_ibgt_tgt_p1 : alu1_ibr_p1 ? alu1_b_p1 : alu1_vs_p1;

  // ── CDB1: LD0 / FPU0 (with FPU stash for collision handling) ──
  ...
  // Store-to-load forwarding: pick most recent OLDER store with matching address
  integer sf;
  reg fwd_hit;
  reg [63:0] fwd_val;
  reg [ROB_BITS-1:0] fwd_best;
  always @(*) begin
    fwd_hit  = 0;
    fwd_val  = 64'd0;
    fwd_best = 0;
    ...
    if (lsq_v[sf] && lsq_st[sf] && lsq_ardy[sf] && lsq_drdy[sf] &&
      ...
      // only forward from stores older than the load (smaller ROB distance from head)
      if ((lsq_rob[sf][ROB_BITS-1:0] - rob_head) < (lsq_rob[lsq_head][ROB_BITS-1:0] - rob_head)) begin
        if (!fwd_hit || ((lsq_rob[sf][ROB_BITS-1:0] - rob_head) > (fwd_best - rob_head))) begin
          fwd_hit  = 1;
          fwd_val  = lsq_data[sf];
          fwd_best = lsq_rob[sf][ROB_BITS-1:0];
        end
      end
    end
  end
  ...
  always @(*) begin
    ...
    if (lsq_v[sf2] && lsq_st[sf2] && lsq_ardy[sf2] && lsq_drdy[sf2] &&
      ...
      if ((lsq_rob[sf2][ROB_BITS-1:0] - rob_head) < (lsq_rob[lsq_head2][ROB_BITS-1:0] - rob_head)) begin
        if (!fwd_hit2 || ((lsq_rob[sf2][ROB_BITS-1:0] - rob_head) > (fwd_best2 - rob_head))) begin
          fwd_hit2  = 1;
          fwd_val2  = lsq_data[sf2];
          fwd_best2 = lsq_rob[sf2][ROB_BITS-1:0];
        end
      end
    end
  end
  ...
  reg        call_pending;
  reg [63:0] call_tgt, call_addr, call_wdata;
  reg        ret_pending;
  reg [63:0] ret_addr;
  ...
  // RS issue: two ready slots
  integer rs_iss_idx, rs_iss_idx2;
  reg rs_iss_found, rs_iss_found2;
  always @(*) begin
    rs_iss_idx   = 0;
    rs_iss_found = 0;
  ...
  always @(*) begin
    ...
    if (rs_iss_found)
      ...
      if (!rs_iss_found2 && rs_v[i] && rs_psrdy[i] && (rs_uimm[i] || rs_ptrdy[i]) &&
            (!rs_ibgt[i] || rs_pt3rdy[i]) && i != rs_iss_idx)
          begin
        rs_iss_idx2   = i;
        rs_iss_found2 = 1;
      end
  ...
  // FP issue: two ready slots
  integer fp_iss_idx, fp_iss_idx2;
  reg fp_iss_found, fp_iss_found2;
  always @(*) begin
    fp_iss_idx   = 0;
    fp_iss_found = 0;
  ...
  // ── Macro to broadcast on a CDB port (avoid code duplication) ──
  // Used inline below for all 4 CDB ports
  ...
  always @(posedge clk) begin
    if (reset) begin
      ...
      // Helper task inlined as macro-style block for CDB0
      ...
      // FPU0 stash: save result when ld_done steals CDB1
      if (ld_done && fpu_vout) begin
        fpu_stash_v   <= 1;
        fpu_stash_pd  <= fp_pd;
        fpu_stash_val <= fpu_res;
        fpu_stash_rob <= fpu_tout[ROB_BITS-1:0];
      end else if (fpu_stash_v && !ld_done) fpu_stash_v <= 0;
      ...
      end else if (fpu1_stash_v && !ld1_done) fpu1_stash_v <= 0;
      ...
      begin : commit_blk
        ...
        if (rob_cnt > 0 && rob_valid[ch] && rob_done[ch]) begin
          ...
          if (do_flush) begin
            ...
            dq_v[3]  <= 0;
            dq_cnt   <= 0;
            commit_freed_reg = 0;  // prevent dispatch_blk from adding +1 to fl_cnt
          end
        end
      end
      ...
      if (!redirect_en && !call_pending && !ret_pending && !flush_this_cycle) begin : dispatch_blk
        ...
        if (d0_en) begin
          ...
          if (c0en) begin
            if (!p0_psrdy && p0_ps == c0pd) begin
              p0_psrdy = 1;
              p0_vs = c0val;
            end
            if (!p0_ptrdy && p0_pt == c0pd) begin
              p0_ptrdy = 1;
              p0_vt = c0val;
            end
          end
          if (c1en) begin
            if (!p0_psrdy && p0_ps == c1pd) begin
              p0_psrdy = 1;
              p0_vs = c1val;
            end
            if (!p0_ptrdy && p0_pt == c1pd) begin
              p0_ptrdy = 1;
              p0_vt = c1val;
            end
          end
          if (c2en) begin
            if (!p0_psrdy && p0_ps == c2pd) begin
              p0_psrdy = 1;
              p0_vs = c2val;
            end
            if (!p0_ptrdy && p0_pt == c2pd) begin
              p0_ptrdy = 1;
              p0_vt = c2val;
            end
          end
          if (c3en) begin
            if (!p0_psrdy && p0_ps == c3pd) begin
              p0_psrdy = 1;
              p0_vs = c3val;
            end
            if (!p0_ptrdy && p0_pt == c3pd) begin
              p0_ptrdy = 1;
              p0_vt = c3val;
            end
          end
        ...
        if (d1_en) begin
          ...
          if (c0en) begin
            if (!p1_psrdy && !(d0_en && d0_wr && d0_rd == d1_rs) && p1_ps == c0pd) begin
              p1_psrdy = 1;
              p1_vs = c0val;
            end
            if (!p1_ptrdy && !(d0_en && d0_wr && d0_rd == d1_rt) && p1_pt == c0pd) begin
              p1_ptrdy = 1;
              p1_vt = c0val;
            end
          end
          if (c1en) begin
            if (!p1_psrdy && !(d0_en && d0_wr && d0_rd == d1_rs) && p1_ps == c1pd) begin
              p1_psrdy = 1;
              p1_vs = c1val;
            end
            if (!p1_ptrdy && !(d0_en && d0_wr && d0_rd == d1_rt) && p1_pt == c1pd) begin
              p1_ptrdy = 1;
              p1_vt = c1val;
            end
          end
          if (c2en) begin
            if (!p1_psrdy && !(d0_en && d0_wr && d0_rd == d1_rs) && p1_ps == c2pd) begin
              p1_psrdy = 1;
              p1_vs = c2val;
            end
            if (!p1_ptrdy && !(d0_en && d0_wr && d0_rd == d1_rt) && p1_pt == c2pd) begin
              p1_ptrdy = 1;
              p1_vt = c2val;
            end
          end
          if (c3en) begin
            if (!p1_psrdy && !(d0_en && d0_wr && d0_rd == d1_rs) && p1_ps == c3pd) begin
              p1_psrdy = 1;
              p1_vs = c3val;
            end
            if (!p1_ptrdy && !(d0_en && d0_wr && d0_rd == d1_rt) && p1_pt == c3pd) begin
              p1_ptrdy = 1;
              p1_vt = c3val;
            end
          end
      ...
      if ((call_pending || ret_pending) && !redirect_en) begin
        if (rs_iss_found2) rs_cnt <= rs_cnt - 2;
        else if (rs_iss_found) rs_cnt <= rs_cnt - 1;
        if (fp_iss_found2) fp_cnt <= fp_cnt - 2;
        else if (fp_iss_found) fp_cnt <= fp_cnt - 1;
        if (lsq_exec2) lsq_cnt <= lsq_cnt - 2;
        else if (lsq_exec) lsq_cnt <= lsq_cnt - 1;
      end
      ...
      if (redirect_en || call_pending || ret_pending) begin
        ...
        if (d0_en && d0_ctrl) begin
          // Branch/ctrl in slot 0: keep slot 1, discard 2..3, fetch one more
          dq_v[0] <= dq_v[1];
          dq_i[0] <= dq_i[1];
          dq_pc[0] <= dq_pc[1];
          ...
          begin : fetch_norm
            ...
            if (d0_en && d1_en) begin
              if (d1_is_ctrl_r) begin
                // d1 was ctrl: discard dq[2..3], fetch fresh pair
                dq_v[2] <= 0;
                dq_v[3] <= 0;
                dq_v[0] <= 1;
                dq_i[0] <= mem_i0;
                ...
                // Normal dual: shift 2..3 -> 0..1, fetch 2 new into 2..3
                dq_v[0] <= dq_v[2];
                dq_i[0] <= dq_i[2];
                dq_pc[0] <= dq_pc[2];
              ...
              // Only d0 dispatched: shift 1..3->0..2, fetch 1 into 3
              dq_v[0] <= dq_v[1];
              dq_i[0] <= dq_i[1];
              dq_pc[0] <= dq_pc[1];
              dq_ptaken[0] <= dq_ptaken[1];
              ...
              // DQ empty: fill with up to 4 instructions
              dq_v[0] <= 1;
              dq_i[0] <= mem_i0;
              dq_pc[0] <= pc_reg;
              dq_ptaken[0] <= bp_pred_taken0;
