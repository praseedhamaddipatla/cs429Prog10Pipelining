// fpu.sv — pipelined fp units (3-stage each)
// Yosys-compatible: no while loops, no typed localparams
...
module fpu (
    ...
    fpu_addsub u_addsub (
        ...
        .a(a), .b(b), .do_sub(op==FSUB),
        .rob_tag_in(rob_tag_in),
        .valid_out(add_valid), .result(add_res), .rob_tag_out(add_tag)
    );
    fpu_mul u_mul (
        .clk(clk), .reset(reset),
        .valid_in(valid_in && op==FMUL),
        .a(a), .b(b), .rob_tag_in(rob_tag_in),
        .valid_out(mul_valid), .result(mul_res), .rob_tag_out(mul_tag)
    );
    fpu_div u_div (
        .clk(clk), .reset(reset),
        .valid_in(valid_in && op==FDIV),
        .a(a), .b(b), .rob_tag_in(rob_tag_in),
        .valid_out(div_valid), .result(div_res), .rob_tag_out(div_tag)
    );

    assign valid_out    = add_valid | mul_valid | div_valid;
    assign result       = add_valid ? add_res : mul_valid ? mul_res : div_valid ? div_res : 64'd0;
    assign rob_tag_out  = add_valid ? add_tag : mul_valid ? mul_tag : div_valid ? div_tag : 6'd0;
endmodule

// ---------------------------------------------------------------------------
// fpu_addsub — 3-stage pipelined fp add/sub
// FIXED: replaced while-loop normalization with fixed-bound for loop
// ---------------------------------------------------------------------------
module fpu_addsub (
    input         clk,
    input         reset,
    ...
    // Stage 2: normalize + round  — FIXED: while replaced with fixed-bound for
    always @(posedge clk) begin
        if (reset) begin
            valid_out <= 0;
        end else begin
            ...
            if (s1_special) begin
                ...
                if (s[55]) begin
                    ...
                    // FIXED: was while(s[54]==0 && s!=0 && k<56)
...
// ---------------------------------------------------------------------------
// fpu_mul — 3-stage pipelined fp multiply (no changes needed, no while loops)
// ---------------------------------------------------------------------------
module fpu_mul (
    input         clk,
    input         reset,
    ...
    always @(posedge clk) begin
        if (reset) begin s0_valid <= 0;
        end else begin
            s0_valid <= valid_in; s0_tag <= rob_tag_in;
            ...
            if (valid_in) begin : s0l
                ...
                sr=sx^sy; er=ex+ey-11'd1023;
                xnan=(ex==11'h7FF)&&(a[51:0]!=0); ynan=(ey==11'h7FF)&&(b[51:0]!=0);
                xinf=(ex==11'h7FF)&&(a[51:0]==0); yinf=(ey==11'h7FF)&&(b[51:0]==0);
                xz=(ex==0)&&(a[51:0]==0); yz=(ey==0)&&(b[51:0]==0);
                if      (xnan)                   begin s0_special<=1; s0_special_val<=a; end
                else if (ynan)                   begin s0_special<=1; s0_special_val<=b; end
                else if ((xinf&&yz)||(xz&&yinf)) begin s0_special<=1; s0_special_val<=64'h7FF8000000000000; end
                ...
                else begin s0_sr<=sr; s0_er<=er; s0_mx<=mx; s0_my<=my; end
            end
        end
    end
    ...
    always @(posedge clk) begin
        if (reset) begin s1_valid<=0; end
            ...
            s1_valid<=s0_valid; s1_tag<=s0_tag;
            s1_special<=s0_special; s1_special_val<=s0_special_val;
            s1_sr<=s0_sr; s1_er<=s0_er; s1_prod<=s0_mx*s0_my;
        end
    end
    ...
    always @(posedge clk) begin
        if (reset) begin valid_out<=0; end
            ...
            valid_out<=s1_valid; rob_tag_out<=s1_tag;
            if (s1_special) begin result<=s1_special_val;
            end else begin : s2l
                reg [53:0] mr; reg [10:0] er; reg guard, rb, sticky;
                er=s1_er;
                if (s1_prod[105]) begin
                    mr={1'b0,s1_prod[105:53]}; guard=s1_prod[52]; rb=s1_prod[51]; sticky=|s1_prod[50:0]; er=er+1;
                end else begin
                    mr={1'b0,s1_prod[104:52]}; guard=s1_prod[51]; rb=s1_prod[50]; sticky=|s1_prod[49:0];
                end
                if (guard&&(rb||sticky||mr[0])) mr=mr+54'd1;
                if (mr[53]) begin mr=mr>>1; er=er+1; end
                result<={s1_sr,er,mr[51:0]};
...
// ---------------------------------------------------------------------------
// fpu_div — 3-stage pipelined fp divide (no while loops to fix)
// ---------------------------------------------------------------------------
module fpu_div (
    input         clk,
    input         reset,
    ...
    always @(posedge clk) begin
        if (reset) begin s0_valid<=0; end
            ...
            if (valid_in) begin : s0l
                ...
                xnan=(a[62:52]==11'h7FF)&&(a[51:0]!=0); ynan=(b[62:52]==11'h7FF)&&(b[51:0]!=0);
                xinf=(a[62:52]==11'h7FF)&&(a[51:0]==0); yinf=(b[62:52]==11'h7FF)&&(b[51:0]==0);
                xz=(a[62:52]==0)&&(a[51:0]==0); yz=(b[62:52]==0)&&(b[51:0]==0);
    ...
    always @(posedge clk) begin
        if (reset) begin valid_out<=0; end
            ...
            if (s1_special) begin result<=s1_special_val;
                ...
                reg [53:0] mr; reg [12:0] er; reg guard, rb, sticky;
                er=s1_er;
                if (s1_qr[53]) begin
                    mr={1'b0,s1_qr[53:1]}; guard=s1_qr[0]; rb=1'b0; sticky=(s1_rem!=0); er=er+1;
                end else begin
                    mr={1'b0,s1_qr[52:0]}; guard=1'b0; rb=1'b0; sticky=(s1_rem!=0);
                end
                if (guard&&(rb||sticky||mr[0])) mr=mr+54'd1;
                if (mr[53]) begin mr=mr>>1; er=er+1; end
